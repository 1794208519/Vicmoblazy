...
$.validator.addMethod("account", function(value, element, param){
    ...
    return new RegExp(/^[a-zA-Z]{1}[-_a-zA-Z0-9]{5,19}$/).test(value);

}, "账号由6~20个字母、数字、下划线和减号组成，且必须以字母开头");

$.validator.addMethod("password", function(value, element, param){
    //方法中又有三个参数:value:被验证的值， element:当前验证的dom对象，param:参数(多个即是数组)
    //alert(value + "," + $(element).val() + "," + param[0] + "," + param[1]);
    return new RegExp(/^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z!#$%^&*\s]{6,16}$/).test(value);

}, "密码必须是6~16为的数字、字符组合（不能是纯数字）");
$.validator.addMethod("devices", function(value, element, param){
    //方法中又有三个参数:value:被验证的值， element:当前验证的dom对象，param:参数(多个即是数组)
    //alert(value + "," + $(element).val() + "," + param[0] + "," + param[1]);
