...
var ExistMsg = 0,
    ...
    $.SmartMessageBox = function (settings, callback) {
        ...
        //如果当前存在Alert,着禁止显示新的Alert
        if (SmartMSGboxCount > 0) return;
        ...
        for (var i = 0; i <= settings.buttons.length - 1; i++) {
            ...
            if (settings.buttons[i] == "[") {
                ...
                if (settings.buttons[i] == "]") {
                    ...
                    } else if (btnText.charAt(0) == '!') {
                        btnType = "btn-danger";
                        btnText = btnText.substring(1);
                    }

                    Name = "<button id='bot" + NumBottons + "-Msg" + SmartMSGboxCount +
                        "' class='btn " + btnType + " btn-sm botTempo'> " + btnText + "</button>";
                    Content += Name;
                } else {
        ...
        $(".botTempo").click(function () {
            ...
            if (SmartMSGboxCount == 0) {
                //Try to fix the smart box bug,the delay remove and exitmsg=0 code will cause some problem.
                $("#MsgBoxBack").remove();
                ExistMsg = 0;
            }
        });
