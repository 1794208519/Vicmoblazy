...
(function ($, window, document) {
    ...
    $.validator.addMethod("identity", function(value, element, param) {
        var result = /^\d{17}[0-9xX]$/.test(value);
    ...
    $.validator.addMethod("phone", function (value, element, param) {
        var result = /^\d{3,4}\-\d{6,8}$/.test(value);
        return this.optional(element) || result;
    }, "错误的电话格式(区号-号码)");

    //中文版本的手机号码检查.
    $.validator.addMethod("mobile", function (value, element, param) {
        var result = /^(1[3578][01379]\d{8}|1[34578][01256]\d{8}|134[012345678]\d{7}|1[34578][012356789]\d{8})$/.test(value);
        return this.optional(element) || result;
    }, "错误的手机号码");
    ...
    $.validator.addMethod("mindigits", function (value, element, param) {
        return this.optional( element ) || ( value * 1 >= param * 1  );
    }, "输入的数值必须大于{0}");
