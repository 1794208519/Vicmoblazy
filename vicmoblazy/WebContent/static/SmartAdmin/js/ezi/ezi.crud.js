...
(function ($, window, document) {
    ...
    var CrudController = function (options) {
        ...
        //deleteurl: 提交删除的url
        //saveurl: 提交保存的url
        //deleteSubMessage：做删除操作时，需要额外提醒客户的信息
        ...
        self.deleteSubMessage = options.deleteSubMessage;
        ...
        self.edit = function (e) {
            ...
            $(self.editForm).eziValidate("refresh");//重置验证
            self.viewModel.EditMode(true);
        };
        ...
        self.delete = function (e) {
            ...
            var tableRow = table.row($row);
            var data = tableRow.data();
            ezi.msg.confirm("是否确认删除: " + self.viewModel.GetName(data) + " ?", self.deleteSubMessage, function () {
        ...
        self.save = function (event,ignorevalid) {
                ...
                ezi.postjson(options.saveurl, data, function (result) {
                    if (result.status) {
                        if (self.editRowIndex != -1) {
                            //更新数据
                            table.updateRow(self.editRowIndex, result.content);
                        } else {
                            //添加数据
                            table.insertRow(result.content);
                        }
